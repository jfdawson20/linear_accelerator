...
class Simulation():
    ...
    def showResults(self,res,verbose=False,plots=False):
        ...
        for s in res:
            ...
            x.add_row([st,s[st]["COIL_TIME"],peakT,minV,peakC,s[0]["INDUCTANCE"],peakV])
            #print(s[len(s)-1])
    ...
    def buildCoilConfigs(self,voltage,capacitance,switchR,min_i, max_i, length, shaft_dia, max_coil_dia, proj_dia):
        minT = 100
        maxT = 400
        turnSteps = 4
        #iterate over turns
        cfgs = []
        for x in range(minT,maxT,turnSteps):
            for g in wire_dict:
                cfg = {}
                cfg["profile"]  = 0
                cfg["cid"]      = 0
                ...
                proj  = Projectile(0.00569)
                coilC = CoilCircuit(0,voltage,capacitance,switchR,proj,cfg)

                i,t = coilC.timeToMaxI()
                
                if(min_i <= i and i <= max_i): 
                    cfgs.append((cfg,coilC))    
        
        return cfgs

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multi-Stage Linear Accelerator simulator/optimizer")
    ...
    parser.add_argument('-sp','--stagespace',dest='stageSpace',help="spacing between stages (in m)",default=0.0188,type=float)
    ...
    if(args.simOp == "exec"):
        ...
        cfgs = sim.buildCoilConfigs(args.capV,args.cap,0.0012,300, 400, args.coilLength, args.barrelOD, 0.05, 0.006)
        print(len(cfgs))
        for c in cfgs:
            print(c[1].Coil.calcInductance(0,100),c[0]["turns"],c[0]["gauge"])
